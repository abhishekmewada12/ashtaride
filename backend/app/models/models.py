...
class RideRequest(Base, TimestampMixin):
    ...
    current_offered_rider_id = Column(UUID(as_uuid=True), ForeignKey("riders.id"), nullable=True)
    offer_expires_at         = Column(DateTime(timezone=True), nullable=True)
    notified_riders          = Column(ARRAY(UUID(as_uuid=True)), default=list)
    rejected_rider_ids       = Column(ARRAY(UUID(as_uuid=True)), default=list)
